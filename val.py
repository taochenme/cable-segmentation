...
def get_parser():
    ...
    parser.add_argument('opts', help='see config/stratified_transformer_train_val_test for all options', default=None, nargs=argparse.REMAINDER)
...
def data_prepare():
    ...
    data_list = [item for item in data_list if int(item[5:]) in args.test_areas]
...
def test(model, criterion, names, test_transform_set):
    ...
    for idx, item in enumerate(data_list):
        ...
        pred_txt, label_txt = np.zeros(shape=(coord.shape[0], 7)), np.zeros(shape=(coord.shape[0], 7))
        pred_txt[:, :3], label_txt[:, :3] = coord, coord
        pred_txt[:, 3:6], label_txt[:, 3:6] = feat, feat
        pred_txt[:, 6] = pred
        label_txt[:, 6] = label
        ...
        if not os.path.isfile(os.path.join(args.save_folder, '{}_{}_label.txt'.format(item, args.epoch))):
            np.savetxt(os.path.join(args.save_folder, '{}_{}_label.txt'.format(item, args.epoch)), label_txt, delimiter=' ')
