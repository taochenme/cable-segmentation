...
class CABLE_SEG(Dataset):
    def __init__(self, split='train', data_root='trainval', test_areas=[1,2], voxel_size=0.02, voxel_max=None, transform=None, shuffle_index=False, loop=1):
        super().__init__()
        ...
        if split == 'train':
            self.data_list = [item for item in data_list if int(item[5:]) not in test_areas]
        else:
            self.data_list = [item for item in data_list if int(item[5:]) in test_areas]
...
if __name__ == '__main__':
    ...
    test_areas, voxel_size, voxel_max = [1,2], 0.02, 8000000

    point_data = CABLE_SEG(split='train', data_root=data_root, test_area=test_area, voxel_size=voxel_size, voxel_max=voxel_max)
