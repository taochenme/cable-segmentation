...
def get_indice_pairs(p2v_map, counts, new_p2v_map, new_counts, downsample_idx, batch, xyz, window_size, i):
    ...
    xyz_min = xyz.min(0)[0]
    if i % 2 == 0:
        # window_coord = torch.div((xyz[new_p2v_map] - xyz_min), window_size, rounding_mode='floor')
        window_coord = (xyz[new_p2v_map] - xyz_min) // window_size #[n, k, 3]
    else:
        # window_coord = torch.div((xyz[new_p2v_map] + 1/2*window_size - xyz_min), window_size, rounding_mode='floor')
...
class WindowAttention(nn.Module):
    ...
    def forward(self, feats, xyz, index_0, index_1, index_0_offsets, n_max):
        ...
        # relative_position_index = torch.div((relative_position + 2 * self.window_size - 0.0001), self.quant_size, rounding_mode='floor')
...
class Stratified(nn.Module):
    def __init__(self, downsample_scale, depths, channels, num_heads, window_size, up_k, \
            ...
            num_layers=4, concat_xyz=False, num_classes=3, ratio=0.25, k=16, prev_grid_size=0.04, sigma=1.0, stem_transformer=False):
        super().__init__()
