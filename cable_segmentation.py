...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/stratified_transformer_train_val_test.yaml', help='config file')
    parser.add_argument('opts', help='see config/stratified_transformer_train_val_test.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def main():
    global args, logger
    args = get_parser()
    logger = get_logger(os.path.join(args.save_folder, 'val.txt'))
    logger.info(args)
...
def data_prepare():
    ...
    data_list = [item[:-4] for item in data_list if '.pcd' in item]
    print("Totally {} samples to be tested.".format(len(data_list)))
    return data_list

def pcd_preprocessing(data_name, transform):
    """read point cloud file"""
    data_path = os.path.join(args.data_root, data_name + '.pcd')
    pcd = o3d.io.read_point_cloud(data_path, remove_nan_points=True)

    """voxel down sample"""
    downpcd = pcd.voxel_down_sample(voxel_size = 1)

    # """plane segmentation"""
    # plane_model, inliers = downsampled.segment_plane(distance_threshold=2.5,
    #                                                 ransac_n=3,
    #                                                 num_iterations=1000)
    # outliers = downsampled.select_by_index(inliers, invert=True)
    # inlier_cloud = downsampled.select_by_index(inliers)

    """remove outlier"""
    cl, ind = downpcd.remove_radius_outlier(nb_points=64, radius=12)
    inlier_cloud = downpcd.select_by_index(ind)

    coord = np.asarray(inlier_cloud.points)/100
    feat = np.asarray(inlier_cloud.colors)
...
def test(model, criterion, names, test_transform_set):
    ...
    for idx, item in enumerate(data_list):
        ...
        pred_save_path = os.path.join(args.save_folder, '{}_{}_pred.txt'.format(item, args.epoch))
        # label_save_path = os.path.join(args.save_folder, '{}_{}_label.txt'.format(item, args.epoch))
        
        if os.path.isfile(pred_save_path):
            logger.info('{}/{}: {}, loaded pred.'.format(idx + 1, len(data_list), item))
            ...
            for aug_id in range(len(test_transform_set)):
                ...
                if os.path.isfile(pred_save_path):
                    ...
                    coord, feat, idx_data = pcd_preprocessing(item, test_transform)
        ...
        batch_time.update(time.time() - end)
        logger.info('Test: [{}/{}] '
                    'Batch {batch_time.val:.3f} ({batch_time.avg:.3f}) '.format(idx + 1, len(data_list), batch_time=batch_time))
        ...
        # data = np.load(os.path.join(args.data_root, item + '.npy'))
        # pred = np.load(pred_save_path)
        
        pred_txt = np.zeros(shape=(coord.shape[0], 7))
        pred_txt[:, :3] = coord*100
        pred_txt[:, 3:6] = feat
        pred_txt[:, 6] = pred

        if not os.path.isfile(os.path.join(args.save_folder, '{}_{}_pred.txt')):
            np.savetxt(os.path.join(args.save_folder, '{}_{}_pred.txt'.format(item, args.epoch)), pred_txt, delimiter=' ')
